...
dim(raw_data_countries)

names(raw_data_countries)

# grain of data is gender, age_bucket, date, country
raw_data_countries[1:10, 1:10]
...
for(j in 9:ncol(raw_data_countries)){
  raw_data_countries$tmp <- pull(raw_data_countries, j)
  p0 <- raw_data_countries %>%
    ...
    labs(y = names(raw_data_countries)[j],
         title = names(raw_data_countries)[j],
         x = "Date in 2020")
